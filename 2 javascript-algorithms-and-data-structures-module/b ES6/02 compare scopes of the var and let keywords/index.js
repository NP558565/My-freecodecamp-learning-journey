...
:zap: The `let` keyword behaves similarly, but with some extra features. When you 
...
later use inside a for loop that uses the i variable. This is because the stored 
...
// returns 3
```
As you can see, `printNumTwo()` prints `3` and _not_ `2`. This is because 
the value assigned to `i` was updated and the `printNumTwo()` returns the
...
variable than `i` declared in the first line of the function. Be certain not 
to use the `var` keyword anywhere in your code.
...
and `let` keywords assign scope to the declared variable. When programming
 a function similar to the one used in this exercise, it is often better to
